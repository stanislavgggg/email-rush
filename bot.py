...
# Show the privacy link ONLY when a real URL is configured (brand default or PRIVACY_URL env).
# Prevents posting a wrong-product / placeholder link to users and ad-moderation reviewers.
...
def _priv_md(lang: str) -> str:
    return f"\n[{_PRIV_LABEL.get(lang, _PRIV_LABEL['en'])}]({PRIVACY_URL})" if _SHOW_PRIVACY else ""

def _priv_html(lang: str) -> str:
    return f'\n<a href="{PRIVACY_URL}">{_PRIV_LABEL.get(lang, _PRIV_LABEL["en"])}</a>' if _SHOW_PRIVACY else ""

# Primary /start path (send_pic) renders Markdown → [text](url) hyperlink (only if configured).
START_LEGAL_FOOTER = {
...
# Fallback /start path (hook.png) renders HTML → matching <a href> hyperlink (only if configured).
START_LEGAL_FOOTER_HTML = {
    "en": f'\n\n———\n18+ · Informational only, not betting/financial advice.{_priv_html("en")}',
    "es": f'\n\n———\n18+ · Solo información, no es asesoramiento de apuestas/financiero.{_priv_html("es")}',
    "ru": f'\n\n———\n18+ · Только информация, не беттинг/финансовый совет.{_priv_html("ru")}',
}
